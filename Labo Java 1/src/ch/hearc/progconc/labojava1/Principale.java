 ...
 * retournant un booléean indiquant si un objet se retrouve à l'intérieur. Il y a encore une autre méthode non-abstract, la méthode "exit" qui permet à un objet graphique de sortir de la zone et en laisser un autre (du même type) entrer.
 * <br />
 * Pour les méthodes abstracts il y a :
 * - iWantToEnter, permettant de faire entrer un objet dans la zone
 * - releaseXXX, permettant de libérer un objet de type XXX de la zone.
 * 
 * Nous avons ensuite 2 classes enfants : ProtectedZoneReetrantLock, représentant la zone utilisant le ReetrantLock, puis ProtectedZoneSemaphore, représentant la zone utilisant le Semaphore.
 * <br />
 ...
 * 1) Nous interrompons tous les threads des objets graphiques
 * 2) Nous supprimons tous les objets du vector contenant les objets
 * 3) Nous réinitalisons les zones protégées, avec la méthode init(), qui est aussi utilisée dans le constructeur
 * 
 * 
 ...
 * puis avons généré un grand nombre d'objet. Pour chacune de ces zones, il devait y avoir, au maximum, un object graphique de chaque type
 * à l'intérieur. Les autres objets n'étant pas dedans attendent à l'extérieur.
 * Pour le deuxième test, nous cliquons sur le bouton "Effacer" et refaisons la même manipulation. Si l'ensemble marche à nouveau sans erreur, 
